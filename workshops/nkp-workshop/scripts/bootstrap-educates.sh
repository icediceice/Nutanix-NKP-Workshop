...
clusterPackages:
  contour:
    enabled: false
  certManager:
    enabled: ${cert_mgr_enabled}
  registry:
    enabled: true

workshopSecurity:
  rulesEngine: gatekeeper
EOF
...
deploy_educates_platform() {
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    dry "educates admin platform deploy --config ${config_file} --kubeconfig ${KUBECONFIG_PATH} --skip-cluster-validation"
    return
  fi
...
wait_platform_ready() {
  ...
  # Wait for Educates operator deployment
  wait_for "Educates operator deployment" 300 \
    "k get deployment -n ${EDUCATES_NAMESPACE} educates-cluster-essentials-controller 2>/dev/null | grep -q '1/1'"

  # Wait for registry (if enabled)
  wait_for "Educates in-cluster registry" 180 \
    "k get deployment -n ${EDUCATES_NAMESPACE} registry 2>/dev/null | grep -q '1/1'" || \
    warn "Registry deployment not found — may use different name in this version"
...
publish_workshop() {
  ...
  # Discover in-cluster registry (Educates-managed or standalone)
  local registry_svc registry_ns
  registry_ns="$EDUCATES_NAMESPACE"
  if k get svc -n "$registry_ns" registry &>/dev/null; then
    registry_svc="registry"
  elif k get svc -n "$registry_ns" docker-registry &>/dev/null; then
    registry_svc="docker-registry"
  else
    # Fallback: find any registry service
    registry_svc=$(k get svc -n "$registry_ns" \
      -o jsonpath='{.items[?(@.spec.ports[0].port==5000)].metadata.name}' 2>/dev/null | \
      awk '{print $1}')
    [[ -z "$registry_svc" ]] && error "No registry service found in namespace ${registry_ns}"
  fi

  local local_port="${LOCAL_REGISTRY_PORT:-5001}"
  REGISTRY_HOST="localhost:${local_port}"

  info "Port-forwarding registry: ${registry_ns}/${registry_svc}:5000 → localhost:${local_port}"
  if [[ "$DRY_RUN" == "true" ]]; then
    dry "kubectl port-forward ... & educates publish-workshop --image-repository ${REGISTRY_HOST}"
    return
  fi

  # Start port-forward in background, clean up on exit
  k port-forward -n "$registry_ns" "svc/${registry_svc}" "${local_port}:5000" &>/dev/null &
  local PF_PID=$!
  trap "kill ${PF_PID} 2>/dev/null || true" EXIT INT TERM

  # Wait for port-forward to be ready
  local pf_wait=0
  until curl -sf "http://localhost:${local_port}/v2/" &>/dev/null; do
    (( pf_wait >= 20 )) && error "Port-forward to registry did not become ready"
    sleep 2; (( pf_wait += 2 ))
  done
  success "Registry port-forward ready on localhost:${local_port}"

  # Publish workshop from the workshop root
  info "Publishing workshop image..."
  pushd "$WORKSHOP_ROOT" > /dev/null
  retry educates publish-workshop \
    --image-repository "${REGISTRY_HOST}" \
    --kubeconfig "$KUBECONFIG_PATH"
  popd > /dev/null
  ...
  success "Workshop image published to ${REGISTRY_HOST}"
}
...
verify_e2e() {
  ...
  # 3. WorkshopEnvironment created
  local envs
  envs=$(k get workshopenvironment -A \
    --no-headers 2>/dev/null | grep -c "$WORKSHOP_NAME" || echo "0")
  if (( envs > 0 )); then
    success "WorkshopEnvironment(s): ${envs} active"
  else
    warn "No WorkshopEnvironments found for ${WORKSHOP_NAME}"
    (( fail++ ))
  fi
