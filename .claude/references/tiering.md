...
In Claude, **tier selection IS the effort lever** — there is no separate reasoning-effort parameter. The model you choose determines the depth of reasoning applied.

| Tier | Model | Effective Effort |
|------|-------|-----------------|
| Tier 1 (Opus) | claude-opus-4-6 | High — reserved for escalation |
| Tier 2 (Sonnet) | claude-sonnet-4-6 | Default — use for most work |
| Tier 3 (Haiku) | claude-haiku-4-5-20251001 | Low — routine delegation only |

Start at Tier 2. The decision to escalate tier is the decision to increase effort.

## Context-Switch Escalation

When blocked, treat escalation as a context switch:

1. **Snapshot:** Record current hypothesis, failed attempts, and exact blocker in active plan.
2. **Switch:** Escalate from Tier 2 (Sonnet) to Tier 1 (Opus) and raise effort from `medium` to `high` for a focused subproblem pass.
3. **Reintegrate:** Summarize findings, update plan, then continue main execution at normal effort.
